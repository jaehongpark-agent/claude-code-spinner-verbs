...
일부 이모지는 **ZWJ**(Zero Width Joiner, `U+200D`)로 여러 문자를 합쳐서 만듭니다. 대부분의 에디터에서는 하나의 이모지로 보이지만, 일부 터미널에서는 분리되어 표시될 수 있습니다.

이 프로젝트에서 사용하는 ZWJ 이모지 2개:

| 조합 | 구성 | 결과 |
|------|------|------|
| 😵 + ZWJ + 💫 | Dizzy Face + Dizzy | 😵‍💫 빙글빙글 눈 |
| 🚶 + ZWJ + ♂️ | Person Walking + Male Sign | 🚶‍♂️ 걷는 남자 |

Claude Code 스피너에서는 정상적으로 하나의 이모지로 표시됩니다. 터미널에서 `😵💫`처럼 분리되어 보이더라도 설정 자체는 문제없습니다.

## 이 프로젝트의 동사 목록 활용

이 저장소에는 Claude Code에서 추출한 187개 스피너 동사와 한국어 번역이 포함되어 있습니다.
...
<details>
<summary>전체 187개 단어 settings.json 예시 (클릭하여 펼치기)</summary>
